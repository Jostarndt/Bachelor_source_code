...
Matrix<dim>::cell(MeshWorker::DoFInfo<dim>& dinfo, MeshWorker::IntegrationInfo<dim>& info) const
{
  ...
  if (info.values.size() > 0)
  {
	...
	const double dh_du = (parameters->rho * v * (1+u+u*u*parameters->K)-parameters->rho * u *v*(1+2*parameters->K*u))/((1+u+parameters->K*u*u)*(1+u+parameters->K*u*u));
	const double dh_dv = (parameters->rho*u)/(1+parameters->K*u*u + u);


      Du_ru[k] =factor* parameters->gamma* (1 + dh_du); 
      Dv_ru[k] =factor* parameters->gamma* (dh_dv);
      Dv_rv[k] =factor* parameters->gamma* (dh_du);
      Du_rv[k] =factor* parameters->gamma* (parameters->alpha + dh_dv);
    }
                  ...
                  MeshWorker::IntegrationInfo<dim>& info2) const
{
  ...
  Laplace::ip_matrix(dinfo1.matrix(0, false).matrix,
                     ...
                     info1.fe_values(0),
                     info2.fe_values(0),
                     1.*Laplace::compute_penalty(dinfo1, dinfo2, deg, deg),
