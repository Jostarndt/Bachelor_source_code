...
#ifndef __jost_explicit_h
#define __jost_explicit_h
...
const double h = ((parameters->rho* u* v)/(1+u+parameters->K*u*u));
    rhs0[k] = u + this->timestep * parameters->gamma* (parameters->a - u - h);
    rhs1[k] = v + this->timestep * parameters->gamma* (parameters->alpha * (parameters->b - v) -h);
                            ...
                            IntegrationInfo<dim>& info2) const
{
  ...
  Laplace::ip_residual(dinfo1.vector(0).block(0),
                       ...
                       -1*parameters->alpha0 * this->timestep);
  Laplace::ip_residual(dinfo1.vector(0).block(1),
                       ...
                       info2.gradients[0][1],
                       Laplace::compute_penalty(dinfo1, dinfo2, deg, deg),
                       -1*parameters->alpha1 * this->timestep);
}
}
